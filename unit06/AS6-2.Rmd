...
<br>
```
1 商場上有許多不確定因素，當有不確定職存在時就需要隨機變數，而隨機變數可以幫助我們估計出資料的分配型態，以找出事件發生機率或平均等統計資料，以分析問題，因此當我們對於資料分配型態未知，卻需要對資料進行統計估計時，便需要用到隨機變數

2 隨機變數因沒有固定的值，故我們可以利用機率分配函數求出隨機變數的分配(非求出一個固定值)，找出他的統計資訊

3 可以，當binwidth調整成適當大小，直方圖下方的區間面積就，就是我們要求的機率，單用數值標記無法輕易估計出時間區間機率，必須搭配平滑曲線才能估計出時間區間之機率。
...
6 不一定，因數值標記是標記每一筆資料發生機率，當預測資料為原本數值標記所使用之資料的哇，數值標記就會很精準，但若為不同筆資料的話，因數值標記泛化能力差，故容易失準。準是指模型所估計出事件發生之機率與事件真實發生機率的誤差很小。
...
<br>
```{r}
#1 尋找最適欄寬的方法為由上往下調，直到width不見為止，因此在欄位數16時，是較佳的直方圖，因為此時與平滑曲線最

#2 約莫在0.5

#3 調高頻寬複雜度會降低，調高欄寬則複雜度會提高

#4 通常模型越複雜，它跟訓練資料之間的誤差就越小，但是模型太複雜就會失去一般性，反而不能夠準確的估計訓練資料以外的對象<br>

#5 模型越複雜，它預測＂訓練資料＂就會越準；但是該樣本不一定等於實際分布，當模型太複雜、太像樣本的時候，模型就會失去它的一般性(過度適配)，當用它來預測樣本以外的資料的時候，它的準確性就會變差。
...
#4
#因為由上題可知在3.8~4.8分鐘時發生機率最高，故將此60秒區隔為10秒賭一次，分別為228、238、248、258、268、278秒時投資一次
...
#6 將獲利的期望值和賭金的比值稱為"期望投資報酬率"，期望報酬為"投資成本*失敗機率+獎金*成功機率" 兩者意思不同

#7 兩者策略不同，若只考慮期望報酬未將投資成本計入，將無法得知獲利佔成本之比重，但若考慮期望報酬率則有將成本計入，是較佳的策略目標
...
# Frame
par(cex=0.7) #畫出框框
plot(0,0,xlim=c(40,100),ylim=c(0,1.1),xlab="等待發時間(分鐘)", 
     ylab="密度 or (累計)機率", main="分布、機率與密度")
abline(h=1, col='lightgray', lwd=0.25, lty=2)

# Empirical PDF 
rug(D) #rug準確，直接在哪裡發生就在哪裡畫出來
# Empirical CDF
plot(ecdf(D), cex=0, verticals=T, lwd=2, col='darkgray', add=T) #add=T是指不要重新畫一張圖直接加在上面

# Histogram PDF
Bins = 20             #畫20欄                # no. bins
bx = seq(min(D), max(D), length=Bins+1) # break sequence  #實際上只有19根所以要+1才有20根
hist(D, col="#B3FFFF7F", border="white", ylim=c(0,1.1),
     freq=F, breaks=bx, add=T) #freq=F, breaks=bx, add=T三個重要參數，因為是機率所以freq要是F
...
y = sapply(x, function(i) integrate(PDF, -Inf, i)$value) #integrate為積分，-inf是負無限大
