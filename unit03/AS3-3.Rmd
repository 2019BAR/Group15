...
nrow(CPS)
#131302
>>>>>>> 9859a934450beb076e544a17208edcb35d34162a
```
...
#
table(D$Region)
which.max(table(D$Region))
#
=======
summary(CPS$Industry)%>% sort
...
```{r}
#
1-(sum(as.numeric(D$Citizenship=="Non-Citizen" , na.rm=TRUE))/ nrow(D))
#
```
...
```{r}
#
D3=read.csv("data/CountryCodes.csv")
nrow(D3)
#
```
