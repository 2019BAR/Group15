...
【1.5】 How many observations have value TUE Rin the Arrest variable (this is the number of crimes for which an arrest was made)?
```{r}
<<<<<<< HEAD
sum(Arr_true = data$Arrest == TRUE)
=======
#15536
sum(Arr_true = data$Arrest[data$Arrest == TRUE])
>>>>>>> c647881f9c6adced17e08f5401e740e12319b19c
```

【1.6】 How many observations have a LocationDescription value of ALLEY?
```{r}
#2308
Alley = data[data$LocationDescription =="ALLEY", ]
nrow(Alley)
```
...
```{r}
median(as.Date(data$Date,"%m/%d/%Y"))
=======
```{r}
#
...
【2.2】 What is the month and year of the median date in our dataset? 
```{r}
data$Date [(nrow(data)/2)]
>>>>>>> fdc75c995757a925b0f409e4bc78f5a98553a26e
>>>>>>> c647881f9c6adced17e08f5401e740e12319b19c
```
...
tail(sort(table(mvtWeek1$LocationDescription)))[-4]
```

【4.2】 How many observations are in Top5?
```{r}
#177510
sum(tail(sort(table(mvtWeek1$LocationDescription)))[-4])
```

【4.3】 One of the locations has a much higher arrest rate than the other locations. Which is it? 
```{r}
#STREET 
which.max(table(mvtWeek1$LocationDescription))
```
